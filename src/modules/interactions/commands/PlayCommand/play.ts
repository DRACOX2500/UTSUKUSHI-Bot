...
import { SlashCommandBuilder, InteractionResponse, ButtonInteraction, ChatInputCommandInteraction, Guild } from 'discord.js';
...
export class PlayCommand implements UtsukushiCommand<ChatInputCommandInteraction> {
	...
	readonly result = async (interaction: ChatInputCommandInteraction, client: BotClient): Promise<void> => {
		...
		let url: string = <string>interaction.options.get('song')?.value?.toString();
		const opti: boolean = !!interaction.options.get('optimization')?.value ?? false;
		...
		stream.setInfoEvent(async (info: any) => {
			...
			client.getDatabase().setCacheByGuild((<any>interaction).guild, { lastPlayURL: stream.source.url });
		...
		client.connection.newBotPlayer((<any>interaction).message)?.playMusic(stream.get(), opti);
	};
