...
export class FuelApiService {
    ...
    static getFuel(fuel: keyof typeof FuelEnum = 'Gazole') {
        return FuelEnum[fuel];
    }

    static get fuelsSearchChoice() {
        const choices: { name: FuelSearch, value: number }[] = []
    ...
    static getFuelSearch(fuelSearch: keyof typeof FuelSearchEnum = 'CITY') {
    ...
    static async search(fuel: Fuel, search: number, value: string): Promise<DataEconomieGouvResponse> {
        ...
        return HttpService.get<DataEconomieGouvResponse>(encodeURI(url));
    }
}