import { HydratedDocument, Model, ObjectId, Query, UpdateQuery } from "mongoose";
...
export abstract class AbstractRecordDocStore<T extends Item = any> extends AbstractRecordStore<HydratedDocument<T>> {
    ...
    private async getOrCreateDoc(search: object, value: T, args: string[] = []) {
    ...
    private async searchDoc(search: object, args: string[] = []): Promise<HydratedDocument<T> | null> {
        const query = this.schema
            .findOne(search);
        return this.populated(query, args).exec();
    }
    ...
    private async getDocs(search: object, args: string[] = []): Promise<HydratedDocument<T>[]> {
        const query = this.schema
            .find(search);
        return this.populated(query, args).exec();
    }
    ...
    async addItem(value: T): Promise<T> {
        ...
        return super.save(doc.id, doc).toObject();
    }
    ...
    async updateItem(id: string, value: UpdateQuery<T> | Partial<T>, args: string[] = []): Promise<T | null> {
        ...
        if (!doc) return null;
        return this.save(doc?.id, doc).toObject() ?? null;
    }
    ...
    async update(id: string, value: Partial<T>): Promise<T | null> {
        return this.updateItem(id, value, []);
    }
